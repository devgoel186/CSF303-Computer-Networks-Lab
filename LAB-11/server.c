...
void *handle_client(void *arg)
{
  ...
  // char init_message[BUFFER_SZ];
  // sprintf(init_message, "%d", cli->uid);
  // printf("%s", init_message);
  // send_message(init_message, cli);
  // bzero(init_message, BUFFER_SZ);
  char public_key[100];
  char encrypted[ENCRYPTED_LENGTH] = {};
  sprintf(public_key, "publicKey_client%d.txt", (cli->uid ^ 1) + 1);
  char filename[100];
  ...
  while (1)
  {
    ...
    if (receive > 0)
    {
      if (strlen(buff_out) > 0)
      {
        ...
        if (strcmp(buff_out, "EXIT") != 0)
        {
          ...
          int encrypted_length = public_encrypt(buff_out, strlen(buff_out), public_key, encrypted);
          // printf("%s\n", encrypted);
    ...
    bzero(&encrypted, ENCRYPTED_LENGTH);
    bzero(&buff_out, BUFFER_SZ);
    bzero(&filename, 100);

    // printf("%d - %s - %s - %s\n", cli->uid, encrypted, buff_out, filename);
  }
